...
interface DashboardData {
  ...
  topCampaignsByNetValue: TopCampaignRow[];
...
export function useDashboardData(
  ...
  const fetchData = useCallback(async () => {
    ...
    const allCampaignRows: TopCampaignRow[] = campaigns
      .map((camp) => {
        ...
        return {
          ...
          netValue: totalEMV - totalProdCost,
        };
      });

    const topCampaigns = allCampaignRows
      .sort((a, b) => b.totalImpressions - a.totalImpressions)
      .slice(0, 5);

    const topCampaignsByNetValue = [...allCampaignRows]
